...
export default function DeleteSchool() {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        dispatch(deleteSchool(id))
          .unwrap()
          .then(() => Swal.fire("Deleted!", "The school has been deleted.", "success"))
          .catch((err) => Swal.fire("Error!", err || "Failed to delete school", "error"));
      }
    });
  };
  ...
  return (
    <div className="min-h-screen bg-amber-50 p-6">
      ...
      <p className="text-amber-800 text-lg mb-6">Manage all registered schools.</p>
        ...
        {schools.map((school) => (
          ...
          >
            <div className="flex flex-col gap-3">
              ...
              <h3 className="text-gray-700">CREATED BY: {school.createdBy?.name}</h3>
              <h3 className="text-gray-700">EMAIL: {school.createdBy?.email}</h3>
            </div>
